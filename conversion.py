...
def geo_to_webm(geo_pair):
	...
	lon=geo_pair[1]
	x=lon/360.0+0.5
	y=math.log(math.tan(math.pi/4.0+math.pi*lat/360.0))/(math.log(math.tan(math.pi/4.0+lat_lim*math.pi/360.0)))*(-0.5)+0.5
	return [x,y]
...
def webm_to_geo(webm_pair):
	...
	y=webm_pair[1]
	lon=(x-0.5)*360.0
	lat=(math.atan(math.exp((y-0.5)*2*math.log(math.tan(math.pi/4.0+lat_lim*math.pi/360.0))))-math.pi/4.0)*360.0/math.pi
	return [lat,lon]

def webm_to_pixel(webm_pair):
	x=int(1024.0*webm_pair[0])
	y=int(1024.0*webm_pair[1])
	return [x,y]
